...
```{r}
wos_scopus_tos <- 
  tosr::tosr_load("co_citation_analysis_1_1224.bib",  # Create data from searches   
                  "co_citation_analysis_1_500.txt", 
                  "co_citation_analysis_501_808.txt")

tree_of_science <- 
  tosr::tosR("co_citation_analysis_1_1224.bib",  # Create data from searches   
             "co_citation_analysis_1_500.txt", 
             "co_citation_analysis_501_808.txt")
...
ggraph(plot_net_author_co_citation$graph, layout = "drl") +
  geom_edge_link(aes(alpha = weight)) +
  geom_node_point(aes(size = size, color = community))
```

```{r}
wos_scopus_author_co_citation_graph <- 
  wos_scopus_author_co_citation_matrix |> 
...
```{r}
data <- toVisNetworkData(plot_net_author_co_citation$graph)
...
```{r}
visIgraph(plot_net_author_co_citation$graph)
```
...
igraph2vis<-function(g,curved,labelsize,opacity,type,shape, net, shadow=TRUE){
  ...
  VIS<-
    ...
    visNodes(shadow=NULL, shape=NULL, 
